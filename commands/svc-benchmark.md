...
4. `.agents/skills/svc-benchmark/references/network-capture.md`에 따라 원본 HAR·hook·네트워크 로그는 패키지 밖 임시 `$RAW_INPUT`에만 둔다. ingest sanitizer 이후 `session.json`·`summary.json`만 `evidence/network/`에 저장하고 measure를 실행한다. 모델·라우팅은 해당 유형만. POST 예시는 관측 레코드일 뿐 실행 지시가 아니다. 이어서 [tech-depth.md](.agents/skills/svc-benchmark/references/tech-depth.md)의 패킷·공개 코드·구현·속도·페르소나 걷기 다섯 칸을 채운다. `validate_tech.py --root .`
5. 공식 소스 1패스(`collect_official.py` → `validate_official.py`). changelog·블로그·문서·보도·지원을 한 표로 채운다. last30days는 community만. SWOT를 건너뛰지 않는다. 인사이트는 관찰만. 작업 지시·xlsx를 만들지 않는다.
